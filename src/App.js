...
function App() {
  ...
  const changeCurrentIndex = (index) => {
    if (index !== currentIndex && !isNaN(index)) {
      ...
      // have to use the response data in "objectsData" to call whichApiObject here. But atleast we're not sending a request to the API everytime we change index
      const details = whichApiObject(apiObjectName, index, apiObjectData);
      setObjectDetails(details);
    }
  };

  /* Todo: There is (I believe) a better way to send requests by using the /query path https://github.com/r-spacex/SpaceX-API/blob/master/docs/queries.md
  for example to better include the /launches path (as it is quite big) with pagination, perhaps create a seperate request for each API endpoint instead of just changing 
  the data structure so we only request the data we need. (example: only fetching all "name" (& "index"?) properties for ApiMenuList.js)*/
